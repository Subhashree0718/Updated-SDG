...
class _MenuScreenState extends State<MenuScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Staggered animation for appearing items one by one
  ...
  void _staggeredAnimation() async {
    for (int i = 0; i < menuItems.length; i++) {
      ...
      setState(() {
        displayedItems.add(menuItems[i]);
        _listKey.currentState?.insertItem(displayedItems.length - 1);
      });
    }
  }
  ...
  void updateQuantity(int index, int change) {
    setState(() {
      int newQuantity = displayedItems[index]["quantity"] + change;
      if (newQuantity >= 0) {
        displayedItems[index]["quantity"] = newQuantity;
      }
    });
  }
  ...
  Widget _buildMenuItem(int index) {
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text("Calories: ${displayedItems[index]["calories"]}",
            style: TextStyle(color: Colors.grey[400], fontSize: 14)),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            ...
            Text(
              displayedItems[index]["quantity"].toString(),
              style: const TextStyle(fontSize: 18, color: Colors.white),
            ),
            IconButton(
